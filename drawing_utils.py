...
LIPS = set()
for lip_point in LIP_POINTS:
	LIPS.add(lip_point[0])
	LIPS.add(lip_point[1])
...
def lip_points(
  ...
  """Draws the lips landmarks on the image.
  ...
  for idx, landmark in enumerate(landmark_list.landmark):
    ...
    landmark_px = _normalized_to_pixel_coordinates(landmark.x, landmark.y,
                                                   image_cols, image_rows)
    if idx in LIPS:
        cv2.circle(image, landmark_px, 5, (255, 0, 0), -1)
        lip_points.append(landmark_px)
  return lip_points
...
def handindex_point(
  ...
  for idx, landmark in enumerate(landmark_list.landmark):
    ...
    landmark_px = _normalized_to_pixel_coordinates(landmark.x, landmark.y,
                                                   image_cols, image_rows)
    if idx in INDEX_POINT:
        cv2.circle(image, landmark_px, 5, (255, 0, 0), -1)
        handindex_point.append(landmark_px)
  return handindex_point
